...
//   post {
//     success { script { notifyRelease(env: 'dev', service: 'payment-service', status: 'success') } }
//     failure { script { notifyRelease(env: 'dev', service: 'payment-service', status: 'failed')  } }
//   }
...
def call(Map args) {
    ...
    def payload = [ attachments: [[ color: s.color, blocks: [
        ...
        [type:'section', text:[type:'mrkdwn', text:"*By:* ${triggeredBy ?: 'n/a'}   *Commit:* `${sha ?: 'n/a'}`"]],
